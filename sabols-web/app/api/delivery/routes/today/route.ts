...
export async function GET(req: NextRequest) {
  try {
    ...
    // 1. Find all Routes assigned to this delivery boy for today
    ...
    if (routeInfoRes.rows.length === 0) {
      return NextResponse.json(
        { success: true, message: "No route assigned for today", routes: [] },
        { status: 200 }
      );
    }
    ...
    for (const routeInfo of routeInfoRes.rows) {
      ...
      if (ordersRes.rows.length > 0) {
        ...
        formatOrders = ordersRes.rows.map((row: any) => {
          ...
          const onlinePaidAmountRupees = row.orderPaymentMethod === 'ONLINE'
            ? Math.max(0, (orderAmountInPaise - codAdjustmentAmountPaise) / 100)
            : 0;
    ...
    console.error("Error in GET /api/delivery/routes/today:", error);
    return NextResponse.json(
...
export async function POST(req: NextRequest) {
  try {
    ...
    const routeRes = await query<{ id: string; isSubmitted: boolean }>(
      `SELECT "id", "isSubmitted" FROM "Route" WHERE "deliveryBoyId" = $1 AND "id" = $2 AND "date"::date = $3::date`,
      [deliveryBoyId, routeId, dateStr]
    );
    ...
    console.error("Error in POST /api/delivery/routes/today:", error);
    return NextResponse.json(
      { success: false, message: "Internal server error" },
      { status: 500 }
    );
