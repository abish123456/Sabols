...
export async function PATCH(
  ...
  try {
    ...
    if ((action === 'UPDATE_ADDRESS' || action === 'EDIT_ITEMS') && (routeToken || (shiftStatus && shiftStatus !== 'NOT_STARTED'))) {
      return NextResponse.json(
        { success: false, message: `Cannot ${action === 'EDIT_ITEMS' ? 'edit order items' : 'update address'} after route link is generated or shift has started` },
        { status: 400 }
      );
    }
    ...
    if (action === 'EDIT_ITEMS') {
      ...
      const committedQtyRes = await query<{ committedOrdered: string, committedReturned: string }>(`
        SELECT 
          COALESCE(SUM(o."quantity"), 0)::bigint as "committedOrdered",
